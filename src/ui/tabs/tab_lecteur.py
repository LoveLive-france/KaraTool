...
class TabLecteur(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        ctk.CTkButton(controls, text="Ouvrir média", command=self.open_media).pack(side="left", padx=10)
    ...
    def open_media(self):
        path = filedialog.askopenfilename(
            filetypes=[("Audio/Video", "*.mp3 *.wav *.flac *.mp4 *.mkv *.avi"), ("All", "*.*")]
        )
        if path:
    ...
    def reload_media(self):
        ...
        # 1. On récupère le chemin de l'image ET la durée du MP3
        cover_path, duration = self.get_audio_info(self.media_path)
        
        # 2. On passe la durée au manager
        self._manager.load_media(cover_path, self.media_path, self.subtitle_path, duration)
    ...
    def toggle_play(self):
        if self._manager.player.is_playing(): self._manager.pause()
        else: self._manager.play()
    ...
    def on_key(self, event):
        ...
        if key == "space": self.toggle_play()
        elif key == "left": self.seek(-5000)
        elif key == "right": self.seek(5000)
        elif key == "up": self.change_volume(5)
        elif key == "down": self.change_volume(-5)
        elif key == "s": self._manager.stop()
        elif key == "g": self.adjust_subtitle_delay(-100)
        elif key == "h": self.adjust_subtitle_delay(100)